...
- **Reminders** — The bell in a note's header, or on a checklist item, sets when to be reminded (quick picks: in an hour, this evening, tomorrow 09:00, next Monday); NoteApp shows a Windows notification at that time — or at its next start if it was closed — and the Reminders page (bell in the rail) lists what is overdue, today, tomorrow and later
- **Quick notes from anywhere** — `Ctrl+Alt+N` in any application (or the notification-area icon's Quick note) opens a small window; `Ctrl+Enter` saves it as a note titled by its first line
- **Notification area** — NoteApp's icon near the clock opens it, writes a quick note or quits; optionally (Settings) closing the window leaves NoteApp running there, with any open encrypted note locked
