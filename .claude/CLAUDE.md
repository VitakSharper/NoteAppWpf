...
- **Clickable links** — `Services/TextLinks.cs` (pure, unit-tested) finds web addresses (`http(s)://`, `www.`) and only ever yields a `LinkUrl`, so nothing but HTTP/HTTPS reaches `Services/LinkLauncher.Open(LinkUrl)` (a pasted `file:` hyperlink never runs). In a text block `Services/RichTextLinks.Linkify` wraps them into `Hyperlink`s **without** `NavigateUri` — their text is their target, so fixing a typo fixes the link; a pasted link keeps its own target. It runs on load, in `SyncBlock`, after a paste, and deferred (Background) from `OnRichTextChanged` when the change inserted whitespace — never on `UndoAction.Undo/Redo`, or undoing a link would bring it straight back. Do not trigger it from `PreviewKeyDown`: the space arrives later as `WM_CHAR`, the link then lands before it and Ctrl+Z undoes them in the wrong order. The document stays `IsDocumentEnabled=false` so a plain click edits a link; `OnLinkPreviewMouseLeftButtonDown` handles `Ctrl+Click` for the RichTextBox (`GetPositionFromPoint` lands inside the Run of the hit character) and for checklist / link-block TextBoxes (`GetCharacterIndexFromPoint`); the same `LinkAt` drives the hand cursor (`UpdateLinkCursor` from `PreviewMouseMove`, and from the editor's `PreviewKeyDown/Up` on Ctrl so a still mouse updates too) — `ForceCursor`, or the RichTextBox puts its I-beam back. Exports resolve each Hyperlink with the same `RichTextLinks.Target` into `DocText.Link`, and checklist items are cut around their addresses by `DocTextLinks.Split`, so Word and PDF get real links wherever the editor shows one.
