...
- **Comfortable single-column note list** in `NoteListView.xaml`: a `ListBox` with card-style rows showing title + `Preview` snippet + tag chips + date. Under a date sort the rows are grouped (`ViewModels/NoteGroups`, pure: Pinned / Today / Yesterday / Previous 7 days / Previous 30 days / "August 2026", local days from UTC dates) — `NoteListViewModel.ApplyGrouping` puts a `PropertyGroupDescription` on the collection's default view after every sort; `VirtualizingPanel.IsVirtualizingWhenGrouping` keeps the list virtualized. The old card/DataGrid toggle was removed. Tag chips use a static violet palette (`#E6E8FF` fill / `#3A3F8F` text) intentionally legible on both light and dark card surfaces. The type filter (All/Text/File/Link) is a second `ListBox` styled as MD3 filter chips (`FilterChipItemStyle`) whose `SelectedItem` binds `SelectedTypeFilter`; the reload happens in `OnSelectedTypeFilterChanged` (there is no select command), and `HasActiveFilters` drives the single "clear" button next to the search field.
